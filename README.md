...
3. Run `bun run start` (or `npm run start`) and scan the QR code from your local dev server

> Tip: in Expo Go open the Dev Menu and make sure **Fast Refresh** is enabled. You can also use `bun run start:clear` if updates seem stuck (clears Metro cache).
